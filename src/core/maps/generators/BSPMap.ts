...
export class Builder extends MapBuilder {
  ...
  protected isValid(rect: Rectangle): boolean {
    ...
    for (let x = minX; x < maxX; x++) {
      for (let y = minY; y < maxY; y++) {
        if (this.map.collisions.isObstacle({ x, y })) {
          return false;
        }
      }
    }
  ...
  protected drawCorridors(): void {
    for (let i = 0; i < this.rooms.length - 1; i++) {
      const [prev, next] = [this.rooms[i], this.rooms[i + 1]];
      const x1 =
        prev.x1 + (RNG.int.between(1, Math.abs(prev.x1 - prev.x2)) - 1);
      const y1 =
        prev.y1 + (RNG.int.between(1, Math.abs(prev.y1 - prev.y2)) - 1);
      const x2 =
